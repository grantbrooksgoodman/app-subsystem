...
extension Cached: Cacheable {
    ...
    private var canCacheNewValue: Bool {
        ...
        guard currentMemoryUsage < memoryUsageCeiling else {
            Logger.log(
                .init(
                    "Skipping cache value modification to minimize memory footprint.",
                    extraParams: [
                        "CacheKey": key.rawValue,
                        "MemoryFootprintMB": currentMemoryUsage,
                        "MemoryUsageCeilingMB": memoryUsageCeiling,
                    ],
                    metadata: [self, #file, #function, #line]
                ),
                domain: .caches
