...
public struct CoreKit {
    ...
    public struct UI {
        ...
        public func resignFirstResponder(in view: UIView? = nil) {
            Task { @MainActor in
                guard let view else {
                    return uiApplication.allViewControllers.compactMap { firstResponder(in: $0.view) }.forEach { $0.resignFirstResponder() }
                }
        ...
        public func dismissAlertController(animated: Bool = true) {
            mainQueue.async {
                guard uiApplication.isPresentingAlertController,
                      let alertControllers = alertKitConfig.presentationDelegate?.presentedAlertControllers else { return }
                alertControllers.forEach { $0.dismiss(animated: animated) }
            }
        }
    ...
    public struct Utilities {
        ...
        public func clearCaches(domains: [CacheDomain]) -> [CacheDomain] {
            ...
            if domains.contains(.localTranslationArchive) {
                localTranslationArchiver.clearArchive()
                domains.removeAll(where: { $0 == .localTranslationArchive })
            }
